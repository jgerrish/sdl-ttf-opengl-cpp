#ifndef _SDL_TTF_OPENGL_CPP_SDL_TTF_H_
#define _SDL_TTF_OPENGL_CPP_SDL_TTF_H_
...
//! An SDLTTFInitFailedError exception
//!
//! This exception is thrown when SDL TTF initialization fails
class SDLTTFInitFailedError : public runtime_error {
...
#ifndef NO_EXCEPTIONS
class SDLTTF : private sdl_opengl_cpp::MoveChecker {
#else
class SDLTTF : public sdl_opengl_cpp::Errors {
#endif
  friend class SDLTTFTester;

public:
  //! Construct an SDL TTF object
  //!
  //! This creates a new SDL TTF object.
  ...
  SDLTTF(const std::shared_ptr<sdl_opengl_cpp::SDL> &sdl_);

  //! Create a new SDL TTF object with a custom SDLWrapper
  //!
  //! This provides the same functionality as SDLTTF() but can use custom
  //! SDLTTFWrapper classes like mocks
  //!
  //! \param &sdl_ttf_wrapper_ The SDLTTFWrapper object to use for SDLTTF calls
  //!
  //! \throws a SDLInitFailedError if there was an error creating the
  //! SDL object.
  //!
  //! \return A new SDLTTF object
  SDLTTF(const std::shared_ptr<sdl_opengl_cpp::SDL> &sdl_,
  ...
  // move constructor
  SDLTTF(SDLTTF &&) noexcept;

  // move assignment operator
  SDLTTF &operator=(SDLTTF &&) noexcept;
  ...
  //!
  //! \returns the font's height. or -1 on error
  ...
  //! \param fg the foreground color for the text.
  //! \returns a new 8-bit, palettized surface, or NULL if there was an error.
  SDL_Surface *RenderUNICODE_Solid(TTF_Font *font, const Uint16 *text,
                                   SDL_Color fg);
  ...
  //! \param fg the foreground color for the text.
  //! \returns a new 32-bit, ARGB surface, or NULL if there was an error.
  SDL_Surface *RenderUTF8_Blended(TTF_Font *font, const char *text,
                                  SDL_Color fg);
  ...
  //! \param fg the foreground color for the text.
  //! \returns a new 32-bit, ARGB surface, or NULL if there was an error.
  SDL_Surface *RenderUNICODE_Blended(TTF_Font *font, const Uint16 *text,
                                     SDL_Color fg);
...
private:
  ...
  //! An object that holds pointers to the underlying SDL functions
  std::shared_ptr<SDLTTFWrapper> sdl_ttf_wrapper = nullptr;

  //! Constructor helper function that actually builds the SDL TTF object
  void build();
